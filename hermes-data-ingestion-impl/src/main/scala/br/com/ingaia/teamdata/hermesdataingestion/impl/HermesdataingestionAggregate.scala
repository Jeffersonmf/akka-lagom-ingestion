...
/**
  * This provides an event sourced behavior. It has a state, [[HermesdataingestionState]], which
  ...
  */
object HermesdataingestionBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[HermesdataingestionCommand]): Behavior[HermesdataingestionCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, HermesdataingestionEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[HermesdataingestionCommand, HermesdataingestionEvent, HermesdataingestionState](
        persistenceId = persistenceId,
        emptyState = HermesdataingestionState.initial,
...
/**
  ...
  */
case class HermesdataingestionState(message: String, timestamp: String) {
  def applyCommand(cmd: HermesdataingestionCommand): ReplyEffect[HermesdataingestionEvent, HermesdataingestionState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: HermesdataingestionEvent): HermesdataingestionState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[HermesdataingestionEvent, HermesdataingestionState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[HermesdataingestionEvent, HermesdataingestionState] =
    Effect
...
object HermesdataingestionState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: HermesdataingestionState = HermesdataingestionState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[HermesdataingestionCommand]("HermesdataingestionAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[HermesdataingestionState] = Json.format
}

/**
  * This interface defines all the events that the HermesdataingestionAggregate supports.
  */
sealed trait HermesdataingestionEvent extends AggregateEvent[HermesdataingestionEvent] {
  def aggregateTag: AggregateEventTag[HermesdataingestionEvent] = HermesdataingestionEvent.Tag
}

object HermesdataingestionEvent {
  val Tag: AggregateEventTag[HermesdataingestionEvent] = AggregateEventTag[HermesdataingestionEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends HermesdataingestionEvent
...
/**
  ...
  */
trait HermesdataingestionCommandSerializable

/**
  * This interface defines all the commands that the HermesdataingestionAggregate supports.
  */
sealed trait HermesdataingestionCommand
    extends HermesdataingestionCommandSerializable
...
case class UseGreetingMessage(message: String, replyTo: ActorRef[Confirmation])
    extends HermesdataingestionCommand
...
/**
  ...
  * application loader.
  */
object HermesdataingestionSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[HermesdataingestionState],
